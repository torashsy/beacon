...
create or replace function save_push_subscription(
  p_handle text, p_secret text, p_endpoint text, p_p256dh text, p_auth text,
...
begin
  ...
  insert into push_subscriptions(endpoint, handle, p256dh, auth, user_agent, updated_at)
    ...
    on conflict (endpoint) do update set handle=excluded.handle, p256dh=excluded.p256dh,
...
create or replace function delete_push_subscription(
  p_handle text, p_secret text, p_endpoint text)
returns void language plpgsql security definer set search_path = public, extensions as $$
begin
  if not _check_pass(p_handle, p_secret) then raise exception 'auth'; end if;
  delete from push_subscriptions where endpoint=p_endpoint and handle=lower(p_handle);
end $$;
...
begin
  ...
  if coalesce(p_color_theme,'sky') not in ('peach','mint','sky','lilac','citrus','mono') then raise exception 'invalid color theme'; end if;
  if length(coalesce(p_av,'')) > 2000 or length(coalesce(p_bn,'')) > 2000 then raise exception 'image url too long'; end if;
  if cardinality(coalesce(p_tags,'{}'::text[])) > 5 then raise exception 'too many tags'; end if;
  if exists (select 1 from unnest(coalesce(p_tags,'{}'::text[])) t
    where length(trim(t)) not between 1 and 20 or t ~ '[[:space:]#,、/]') then raise exception 'invalid tag'; end if;
  select coalesce(array_agg(tag order by first_pos),'{}'::text[]) into normalized_tags
  from (select lower(trim(t)) tag, min(ord) first_pos
    from unnest(coalesce(p_tags,'{}'::text[])) with ordinality as u(t,ord)
    group by lower(trim(t))) normalized;
  update profiles set name=p_name,bio=p_bio,emoji=p_emoji,theme=p_theme,av_theme=p_av_theme,
    av_url=p_av,bn_url=p_bn,color_theme=coalesce(p_color_theme,'sky'),tags=normalized_tags,
    status=coalesce(p_status,status),status_at=case when p_status is not null and p_status<>coalesce(status,'') then now() else status_at end
    where handle=lower(p_handle);
  update accounts set updated_at=now() where handle=lower(p_handle);
end $$;
...
language sql security definer stable set search_path = public as $$
  ...
  from profiles p join accounts a on a.handle=p.handle
  where lower(trim(p_tag))=any(p.tags) and length(trim(p_tag)) between 1 and 20
    and not exists (select 1 from account_moderation m where m.handle=p.handle and m.suspended)
